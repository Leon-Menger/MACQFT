...
\begin{equation}\langle O_1 ... O_n \rangle := \frac{1}{Z} \int_\FF e^{i/\hbar S(\phi)} O_1(\phi) ... O_n(\phi) D\phi \end{equation}
...
\begin{equation}\HH[S]_{(ij)}(x_0) V^{(i)}_\alpha = 0 \quad \Leftrightarrow \quad V_\alpha \ \text{is a local symmetry} \end{equation}
...
\begin{equation}\omega := \mu(\delta S), \quad \mu = \mu^{ij} \frac{\delta}{\delta \phi^{(i)}} \wedge \frac{\delta}{\delta \phi^{(j)}}, \quad \omega = \mu^{ij} \frac{\delta S}{\delta \phi^{(i)}} \frac{\delta S}{\delta \phi^{(j)}}, \quad \mu^{ij} = - \mu^{ji} \end{equation}
...
\begin{equation}\gf := \widetilde{\gf}/\tf\end{equation}
Notice that $\mathfrak{J}$ is not necessarily a Lie subalgebra. However if
\begin{equation}[\tf, \gf] \subset \tf, \quad \quad [\gf, \gf] = \tf + \gf \end{equation}
...
\begin{definition}
  ...
  \begin{equation}C^k_{CE}(\gf ; \Ci(M)) := \Hom\left(\bigwedge^k \gf, \Ci(M)\right)\cong\bigwedge^k\mathfrak{g}^*\otimes\Ci(M). \end{equation}
  ...
  \begin{equation}d_{CE} \colon C^k_{CE}(\gf ; \Ci(M)) \lra C^{k+1}_{CE}(\gf ; \Ci(M)) \end{equation}
  as
  \begin{enumerate}
    ...
    \begin{equation}d_{CE} (\alpha \wedge \beta) = d_{CE}\alpha \wedge \beta + (-1)^{|\alpha|} \wedge d_{CE} \beta \end{equation}
    where $|\alpha| = k$ if $\alpha \in \bigwedge^k\gf^*$
    \item We extend to $\bigwedge^\bullet \gf^* \otimes \Ci(M)$ by
    \begin{equation}d_{CE}(\omega \otimes f) = d_{CE} \omega \otimes f + (-1)^{|\omega|} \omega \otimes d_{CE} f\end{equation}
  \end{enumerate}
\end{definition}
...
\begin{equation}d_{CE} = C^a V_a - \frac{1}{2} f^d_{ab} C^a C^b \dell{}{C^d} \end{equation}
Now it is immediate to prove the above proposition.\\

The cohomology of the thus arising cochain complex is
\begin{equation}H^k(C_{CE}^\bullet; \Ci(M)) := \frac{\ker(d_{CE}|_{C^k})}{\ker(d_{CE}|_{C^{k-1}})} \end{equation}
...
\begin{rem}
  ...
  \begin{equation}H^\circ (C_{CE}^\bullet; \Ci(M)) \simeq \Ci(M)^\gf \end{equation}
  where $\Ci(M)^\gf$ are $\gf$-invariant smooth functions on $M$.
\end{rem}

On $\bigwedge^\bullet \gf^*$ there are the following natural operations:
\begin{align}
  \forall \alpha \in \gf^* \quad \mathcal{E}(\alpha) \colon \bigwedge^k \gf^* &\lra \bigwedge^{k+1} \gf^*, \\
  \omega &\lmap \alpha \wedge \omega \\
  \forall X \in \gf \quad \imath(X) \colon \bigwedge^k \gf^* &\lra \bigwedge^{k-1} \gf^*, \\
...
\begin{equation}d_{CE} = \mathcal{E}(\alpha^a) \rho(t_a) - \frac{1}{2} \mathcal{E}(\alpha^a) \mathcal{E}(\alpha^b) \imath_{[t_a, t_b]} \end{equation}
...
\begin{equation}C^a C^b = \mathcal{E}(\alpha^a) \alpha^b = - \mathcal{E}(\alpha^b) \alpha^a = - C^b C^a \end{equation}
such that
\begin{equation}d_{CE} = C^a V_a - \frac{1}{2} f_{ab}^d C^a C^b \dell{}{C^d} \end{equation}
...
\begin{equation}\Ci(M \times \gf[1]) \simeq \bigwedge^\bullet \gf^* \otimes \Ci(M) \cong \Hom(\bigwedge^\bullet \gf, \Ci(M)) \end{equation}
where $\gf[1]$ is a graded vector space, concentrated in degree $-1$. This leads us to
\begin{equation}C_{CE}^\bullet (\gf ; \Ci(M)) \simeq \Ci(M \times \gf[1]) \end{equation}
...
\begin{equation}\frac{1}{2} [d_{CE}, d_{CE}]_{\VF(M_{min})} = d_{CE}^2 = 0 \end{equation}
...
\begin{equation}F_{BRST} := F_{min} \times F_{Aux} \end{equation}
Luckily on $F_{Aux}$ there is a natural cohomological vector field:

\begin{lem}
  On $F_{Aux}$ consider the "vector field" $d_{Aux} \overline{c}_a = \lambda_a$ such that $d_{Aux} \lambda_a = 0$ where we are given a basis $\{e^a\}$ of $\gf^*$, the $\overline{c}_a$ are coordinates in $\gf^*[-1]$ and the $\lambda$ are coordinates in $\gf^*$. Then $d^2_{Aux} = 0$ and thus it is cohomological.
\end{lem}
...
\begin{equation}\Ci(F_{BRST}) \simeq \Ci(F) \otimes \bigwedge^\bullet \gf^* \otimes \bigwedge^\bullet \gf \otimes S^\bullet \gf \simeq C_{CE}^\bullet (\gf, \Ci(F)) \otimes \Omega^\bullet_{dR}(\gf^*[-1]) \end{equation}
...
\begin{rem}
  ...
  \begin{equation}\int d\theta = 0 \quad  \quad \int d\theta \theta = 1 \end{equation}
  In $m$ dimensions we have
  \begin{equation}\int d^m \theta := \int d\theta^m \int ... \int d\theta^1 \end{equation}
  such that $\int d^m \theta \ \theta^1 ... \theta^m = 1$ and zero otherwise. Now $V^0 = V_m^\circ \oplus V^1_m$ such that we can decompose
  \begin{equation}f = f^0 + f^1_i \theta^i + f^2_{ij} \theta^i \theta^j + ... \end{equation}
  where $f^k \in \Ci(V^0)$. Now
  \begin{equation}\int_V f d^mx \ d^m \theta = \int_{V^\circ} f^{top} \ d^n x \end{equation}
  Thus changing coordinates in the form $\widetilde{\theta}^i = A^i_j \theta^j$ amounts to
  \begin{equation}\widetilde{\theta}^1 ... \widetilde{\theta}^m = \det(A) \theta^1 ... \theta^m \end{equation}
  Now we enforce
  \begin{equation}\int d^m \widetilde{\theta} \widetilde{\theta}^1 ... \widetilde{\theta}^m = 1 \end{equation}
  such that $d^m \widetilde{\theta} = (\det(A))^{-1} d^m \theta$. In general by pure construction
  \begin{equation}\langle \mu, (f)_{top} \rangle \in \Gamma(M, \bigwedge^n T^* M) \end{equation}
...
\begin{theo}[BRST]
  ...
  \begin{equation}\lrangle{h}_\psi := \frac{\int_{F_{BRST}} \exp(i/\hbar S_{gf})h \tilde{\mu}}{\int_{F_{BRST}} \exp(i/\hbar S_{gf})\tilde{\mu}} \end{equation}
  such that
  \begin{enumerate}
...
\begin{equation}
  \omega^\# \colon T\MM \atob{\sim} T^* \MM[k]
\end{equation}
...
The BV integral vanishes on $\Delta$-exact functions because regular integral vanish on $d$-exact forms. The BV integral does not depend on a particular Lagrangian inside a family of Lagrangians like $\LL_t$ if the function is $\Delta$-closed, since $d$-closed forms have the same property.
...
\begin{definition}[Quantum BV Formalism]
