...
\begin{definition}[Sections and local sections]
  ...
  We often refer to $\mathcal{F}$ as the \textbf{space of fields}. The space of local sections about $p \in U$ is denoted by $\FF_p U$. Note that if we work with a vector bundle, $\mathcal{F}$ inherits a linear structure.
\end{definition}
...
\begin{rem}
  As an interesting exercise, you can show that the above definition does not depend on the choice of coordinate charts. \emph{Hint:} Introduce multiindices $I$ such that if $\{u^\alpha, U \}_\alpha$ is a chart for $F$, we look at
...
\begin{definition}[Jet bundles]
  ...
  and $J^0 F \cong F$ together with the maps
  \begin{align}
...
\begin{prop}
 ...
 for every $k$. Furthermore that maps $\pi^{k-1}_k$ are \emph{surjective} with \emph{surjective tangent map} (submersions).
\end{prop}
...
\label{def:inverse_system}
  ...
  \begin{itemize}
    ...
    \item $f_{ij}:X_j \lra X_i$ for all $i,j$ s.t. $i \leq j$ and
...
\begin{definition}
  Let $\pi: F \lra M$ be a fibre budle and let $J^kF$ denote the space of $k$-jets. Since we are dealing with finite dimensional manifolds, we can consider $\Ci(J^kF, P)$ for some manifold $P$. For every $l\geq k$ there are connecting maps
...
\begin{definition}
  A family of seminorms $\Gamma = \{| \cdot |_i\}_{i \in I}$ on a vector space $V$ defines a \textbf{unique topology} $\mathcal{T}_\Gamma$ compatible with the vector space structure. The neighbourhood base of $\mathcal{T}_\Gamma$ is given by the family
  $$ B_\Gamma := \{ U^J_\epsilon : \epsilon > 0, \ J \subset I \ \text{finite} \} $$
  with $U^J_\epsilon := \{ v \in V : |v|_i < \epsilon \ \forall i \in J \}$
\end{definition}
...
\label{def:Frechet_manifolds}
  A Hausdorff topological space $M$ is a \textbf{Fréchet manifold} if it is equipped with an atlas of homeomorphisms to open sets $U$ of a Fréchet space $V$ such that the transition functions are smooth in the sense that the Gâteaux-derivations $D^{k}f:U^{\times (k+1)} \lra V$ with
...
\begin{proof}
  ...
  the space is Hausdorff which proves the lemma.
\end{proof}
\end{lem}
...
\begin{itemize}
  \item  The tangent bundle $T_xJ^\infty F$ at a point $x\in J^\infty F$ can be seen as the limit of the system $\{ (T_{\pi^k_\infty(x)}J^kF, T\pi_k) \}$ such that the bundle $T(J^\infty F) = \coprod_{x\in J^\infty F} T_x(J^\infty F)$ is modeled on the $T(J^k F)$ and the projection
...
\begin{definition}~
\begin{itemize}
  \item The set of \textbf{horizontal $(r,p)$-forms} is defined as
  $$ \Omega^{(r,p)}_H := \{ \omega \in \Omega^p(J^\infty F) : \forall \chi \in J^\infty F : \omega_\chi (x_1, ..., x_{p-r+1}, \cdot, ..., \cdot) = 0 \ \ |\ \  x_i \in V_\chi(J^\infty F) \}. $$
  Each of these forms has at least $r$ horizontal differentials in each of its terms.
  ...
  \item The set of \textbf{vertical $(s,p)$-forms} are defined for $s = 0,...,p$ as
  $$ \Omega^{(s,p)}_V := \{ \omega \in \Omega^p(J^\infty F) : \theta_1 \wedge \theta_2 \wedge ... \wedge \theta_s \wedge \widetilde \omega \ \ |\ \  \theta_i \in \mathcal{C}, \widetilde \omega \in \Omega^{p-s}(J^\infty F) \}. $$
\end{itemize}
\end{definition}
...
\begin{proof}
  ...
  for all indices $I,i,\alpha$ such that $\xi$ can be constructed inductively from $u^\alpha_{I,i} \circ \xi$. Thus define $\phi := \pi_\infty^0 \circ \xi$ we see that $\xi = j^\infty \phi$.
\end{proof}
\end{lem}
...
\begin{definition}[Forms of bi-degree]
  ...
  $$ \Omega^{r,s}(J^\infty F) := \Omega_V^{r,p}(J^\infty F) \cap \Omega_H^{s,p}(J^\infty F) $$
...
$$ d \theta^\alpha_I = -d u^\alpha_{I,i} \wedge dx^i = - \theta^\alpha_{I,i} \wedge dx^i $$
...
\begin{definition}[Scalar Field Theory]
  Let $(M,g)$ as above and $\FF= \Ci(M)$ seen as the sections of a trivial $\RR$-bundle. Then (free, a.k.a.\ non-interacting) \textbf{Scalar field theory} on $\FF$ is defined with the Lagrangian
