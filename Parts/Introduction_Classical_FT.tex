...
In this section, we will explore classical field theories  in order to give a solid foundation to their concepts, especially locality. While field theories come in many forms like thermodynamics, electrodynamics, general relativity, the standard model of particle physics and even string theory, we will mainly focus on classical Lagrangian field theories.\\
...
\begin{equation}S(q) = \int_{\RR^+} \left( \frac{m}{2} \dot q^i(t) \dot q^i(t) - V(q(t)) \right) dt \end{equation}
...
Now the \textbf{boundary form} $\frac{m}{2}\delta q^i(0) \dot q^i = \alpha$ can be thought of as a 1-form on $T^*\RR^n$ such that the boundary term, using $p^i(0) = \frac{m}{2} \dot q^i$, can be writen as $\omega := \delta \alpha = \delta q^i(0) \delta p^i(0)$ which is a symplectic form.\\
...
We will usually work on a fibre bundle $\pi \colon F \lra M$ over some smooth manifold $M$. For simplicity, we will assume that $M$ is compact and without boundary, furthermore we assume that $M$ is oriented and connected.

\begin{definition}[Sections and local sections]
  A \textbf{section} of $\pi\colon F \lra M$ is a smooth map $\phi\colon  M \lra F$ such that $\pi \circ \phi = \id_M$. We call $\phi$ a \textbf{local section} if it is only defined on an open subset $U \subseteq M$ such that $\pi \circ \phi = \id_U$. We further denote the space of sections by
  \begin{equation}
    \Gamma(M,F)\colon = \{ \phi \in \Ci(M,F) | \pi \circ \phi = \id_M \} \equiv \mathcal{F}
  \end{equation}
...
\begin{definition}
  For $p \in M$ denote by $\Gamma(p)$ the space of local sections whose domain contains $p$.
\end{definition}
...
\begin{equation}j^k_p \phi \colon = [(\phi, p)]_k \end{equation}
...
\begin{equation}\dell{^{|I|}}{x^I} (u^\alpha \circ \phi) \Big|_p \end{equation}
...
\begin{definition}[Jet bundles]
  ...
  \begin{equation}J^k F \colon = \{j^k_p \phi | p \in M, \phi \in \Gamma(p) \}\end{equation}
  ...
  \end{align}
  such that $\pi_k = \pi \circ \pi^0_k$, $\pi^l_k = \pi^l_m \circ \pi^m_k$ for $0 \leq l \leq m \leq k$. Moreover if $\phi$ is a section of our fibre bundle local in some $U\subseteq M$, we define the \textbf{Jet Prolongations}
  \begin{align}
...
The next proposition will go without proof (see Saunders, Ch6).

\begin{prop}
 There exists a sequence of smooth fibre bundles
 \begin{equation}... \lra J^kF \overset{\pi^{k-1}_k}{\lra} J^{k-1}F \lra ... \lra F \lra M \end{equation}
 for every $k$. Furthermore that maps $\pi^{k-1}_k$ are \emph{surjective} with \emph{surjective tangent map} (sumbersions).
\end{prop}

Now if $(U, u)$ is an adapted chart ($U$ ) of the fibre bundle $F$ such that $(x^i, u^\alpha) \equiv u$ and thus
\begin{align}
  \Rightarrow \quad \quad \quad U^k &= \{j^k_p \phi : p \in M, \phi(p) \in U \subset F\}\\
  u^k &= (x^i, u^\alpha, u_I^\alpha)\\
  u^\alpha_I(j^k_p \phi) &= \dell{^{|I|} (u^\alpha \circ \phi)}{x^I}\Big|_p
\end{align}

Where $I$ is a multiindex with $|I| = k$ and $U^0 \cong U$. Now to make precise Jets agreeing "for all $k$" in the above proposition, we need to introduce some extra structures:
...
\label{def:inverse_system}
  ...
  \begin{itemize}
    ...
    \begin{equation}f_{ik} = f_{ij} \circ f_{jk} \end{equation}
  \end{itemize}
  We call this construct an \textbf{inverse system} or \textbf{projective system}.
\end{definition}

Now we denote by $\underset{\longleftarrow}{\lim} \ X_i$ the subset of elements $x \in \Pi_i X_i$ such that $x_i = f_{ij}(x_j) \ \ \forall i \leq j$. We call $\underset{\longleftarrow}{\lim}$ the \textbf{projective/inverse} limit of the inverse system.
...
\begin{equation}f(\sigma) = \widehat f(x^i, u^\alpha, u^\alpha_{i_1}, u^\alpha_{i_1 i_2}, ..., u^\alpha_{i_1 ... i_k})  \end{equation}
...
\begin{definition}
  ...
  \begin{equation}B_\Gamma := \{ U^J_\epsilon | \epsilon > 0, \ J \subset I \ \text{finite} \} \end{equation}
...
\begin{prop}
  ...
  \begin{enumerate}
    ...
    \begin{equation}v = 0 \quad \Leftrightarrow \quad \left( |v|_i = 0 \quad \forall i \in I\right)\end{equation}

    \item If $(V, \Gamma)$ is Hausdorff then it is metrisable iff the family $\Gamma$ is countable (i.e. there exists a metric $d:V\times V \lra \RR_{\geq0}$ s.t.\ the topology induced by $d$ is $\mathcal{T}_\Gamma$).

    \item Convergence of sequences is controlled by the seminorms, i.e.:
    \begin{equation}(v_n)_{n\in\NN} : v_n \lra v \quad \Leftrightarrow \quad |v_n - v|_i \lra 0 \ \ \forall i \in I \end{equation}
...
\label{def:Frechet_manifolds}
  ...
  \begin{equation}Df(u)v := \underset{t \lra 0}{\lim} \frac{f(u+tv)-f(u)}{t} \end{equation}
  are continuous for all $k\in \NN$.
\end{definition}

Applying the above concept to \ref{def:inverse_system} leads us to the following idea:

\begin{lem}
  Inverse limits of normed spaces are Fréchet manifolds.
\begin{proof}
  ...
  \begin{equation}\{v = 0 \ \Leftrightarrow \ f^n_\infty(v) = 0 \ \forall n \in \NN \ \Leftrightarrow \ |f^n_\infty(v)|_n = 0 \ \forall n \in \NN \ \Leftrightarrow \ |v|_n = 0 \ \forall n \in \NN \}  \end{equation}
  which proves that our space is indeed Hausdorff which proves the lemma.
\end{proof}
\end{lem}

Now let $F\lra M$ be a fibre bundle with coordinate charts $(U_a, u_a)$. Consider the induced cover $U^\infty = \{ (\pi^0_\infty)^{-1}(U_a) \}$ and the induced maps $u^\infty_a: (\pi^0_\infty)^{-1}(U_a) \lra \RR^\infty$ of $F$ respecting the $\infty$-jet bundle. This defines an atlas as can be seen in \emph{Saunders 7.2.4.}. We thus arrive at one of the corner stones of our infinite-dimensional analysis:
...
\begin{itemize}
  ...
  \begin{equation}pr_{J^\infty F}: T(J^\infty F) \lra J^\infty F \end{equation}
  ...
  \begin{equation}T(J^\infty F) := \Ci(\RR, J^\infty F) / \sim \end{equation}
  where $c\sim c^\prime$ iff $c(0) = c^\prime (0)$ and further
  \begin{equation}D(\phi\circ c) (0,1) = D(\phi \circ c^\prime) (0,1) \end{equation}
...
\begin{equation}\Omega^\bullet(F) = \Omega^\bullet(J^0F) \lra \Omega^\bullet(J^1F)\lra ... \end{equation}
...
\begin{definition}[Horizontal differential]
  ...
  \begin{equation}d_H f := \dell{f}{x^i} dx^i + \frac{i_1 ! ... i_m !}{k!} \dell{f}{u^\alpha_I} u^\alpha_{I,i} dx^i\end{equation}
...
\begin{align}
  ...
  D_i &:= \dell{}{x^i} + u^\alpha_{I,i} \cdot \partial^I_\alpha
\end{align}
We can thus denote $d_H f = D_i f dx^i$ and further define the \textbf{vertical differential}
\begin{equation}d_V f = (d-d_H) f = \partial^I_\alpha f (d-d_H) u^\alpha_I = \partial^I_\alpha f d_V u^\alpha_I\end{equation}
We can also prove
\begin{align}
  &(D_i f ) (j^\infty \phi) = \dell{}{x^i}(f(j^\infty \phi))\\
...
\begin{itemize}
  ...
  \begin{equation}\Omega^{(p,s)}_H := \{ \omega \in \Omega^p(J^\infty F) : \omega_\chi (x_1, ..., x_{p-s+1}, \cdot, ..., \cdot) = 0 \ \ |\ \  x_i \in V_\chi(J^\infty F) \} \end{equation}

  \item A \textbf{contact form} is a differential $1$-form $\theta$ on $J^\infty F$ that is annihilated by all jets of the form $j^\infty \phi : M \lra J^\infty F$ via pullback, i.e. $(j^\infty \phi)^* \theta = 0$. Their set is denoted by $\mathcal{C}$.

  \item The set of \textbf{vertical $(p,r)$-forms} are defined as
  \begin{equation}\Omega^{(p,r)}_V := \{ \omega \in \Omega^p(J^\infty F) : \theta_1 \wedge \theta_2 \wedge ... \wedge \theta_r \wedge \widetilde \omega \ \ |\ \  \theta_i \text{ a  contact form} \} \end{equation}
\end{itemize}
\end{definition}

Thus in particular $\Omega^{(s,p)}_V$ trivially vanishes for $s > p$ and $\Omega^{(0,p)}_V = \Omega^p$. In the same manner, $\Omega^{(r,p)}_H$ trivially vanishes for $r > p$ and $\Omega^{(0,p)}_H = \Omega^p$. For contact forms, we can attain a helpful extra property as well as a basis forming the space of contact forms:

\begin{theo}
  Contact forms $\mathcal{C}$ form a differential ideal. Its basis is given by $\theta^\alpha_I = du^\alpha_I - u^\alpha_{I,i} dx^i$.
\begin{proof}
  ...
  \end{align}
  Thus we have a basis of the given form, since locally any $1$-form $\theta$ splits to $\theta = d_H f + d_V f$.
\end{proof}
\end{theo}

Thus we obtain $\Omega_V^{s,p} = \mathcal{C}^s \cap \Omega^p$. We can now use the ideal of contact forms to investigate local sections of the bundle $\pi_\infty$:
...
\begin{proof}
  Note that
  \begin{equation}\xi^* d_V u^\alpha_I = 0 \quad \text{iff} \quad \dell{}{x^i} (u^\alpha_I \circ \xi) = u^\alpha_{I,i} \circ \xi \end{equation}
...
\begin{definition}[Forms of bi-degree]
  ...
  \begin{equation}\Omega^{r,s}(J^\infty F) := \Omega_V^{p,r}(J^\infty F) \cap \Omega_H^{s,s}(J^\infty F) \end{equation}
  where $p:= r+s$. A form $\omega \in \Omega^\bullet(J^\infty F)$ is of bi-degree $(r,s)$ iff it is of the form
  \begin{equation}\omega^{I_1 ... I_r}_{\alpha_1 ... \alpha_r ; i_1 ... i_s} \theta^{\alpha_1}_{I_1} \wedge ... \wedge \theta^{\alpha_r}_{I_r} \wedge dx^{i_1} \wedge ... \wedge dx^{i_s}\end{equation}
...
\begin{equation}\Omega^p (J^\infty F) := \bigoplus_{r+s=p} \Omega^{r,s}(J^\infty F) \end{equation}
...
\begin{equation}d \theta^\alpha_I = -d u^\alpha_{I,i} \wedge dx^i = - \theta^\alpha_{I,j} \wedge dx^j \end{equation}
...
\begin{tikzcd}[sep = huge]
  VF \arrow[r,"pr_F"] & F \arrow[d,"\pi"] \\
  M \arrow[u,"\dot\phi_0"] \arrow[ur,"\phi_0"] \arrow[r, "\id"]  & M
\end{tikzcd}
\end{center}

For $\phi \in \Gamma(M,F) = \FF$ consider sections $\Gamma(M, VF)_\phi = \Gamma(M, \phi^*VF)$. An element of $\Gamma(M, VF)_\phi$ is a map that associates to every $x\in M$ a vector $v_{\phi(x)} \in V_{\phi(x)}F$. We can think of $v_\phi \in \Gamma(M, \phi^*VF)$.
...
\begin{equation}E\times F := \{ (e,f) \in E\times F \big| \pi_E(e)=\pi_F(f) \} \end{equation}
...
\end{equation}
As an exercise, you can prove the above chain of isomorphisms. Since we can further write $T(\FF\times M) \cong (T\FF \times M) \times_{\FF\times M} (\FF \times TM)$ we can for the above statements take a pair $(\phi, x) \in \FF \times M$ and denote
\begin{equation}T_{(\phi, x)}(\FF_x M) \cong T_\phi \FF \times T_x M \end{equation}
...
\begin{prop}
  The tangent map
  \begin{equation}T_{(\phi,x)}j^k \colon T_\phi \FF \times T_x M \lra T_{j^k_x \phi} J^k F \end{equation}
  is given by
  \begin{equation}(T_{(\phi, x)} j^k)(\xi_\phi, v_x) := \sum_{|I|=0}^k \dot u^\alpha_I (j^k_x \xi_\phi) \dell{}{u^\alpha_I} + v_x^i \left( \dell{}{x^i} + \sum_{|I|=0}^k u^\alpha_{I,i} (j^{k+1}_x \phi) \dell{}{u^\alpha_I} \right) \end{equation}
\begin{proof}
  ...
  \begin{equation}\dd{}{t}\left( u^\alpha_I (j^k(\phi_t, x(t))) \right)\Big|_{t=0} = \dd{}{t}\left( \dell{^{|I|}}{x^I} \phi^\alpha(t, x(t)) \right)\Bigg|_{t=0} \end{equation}
...
\begin{corollary}
  ...
  \begin{equation}V = V^i \dell{}{x^i} + \sum_{|I|=0}^\infty V^\alpha_I \dell{}{u^\alpha_I} \end{equation}
  where both $V^i$ and $V^\alpha_I$ are in $\Ci(J^\infty F)$.
\end{corollary}

Now we can finally unambigously answer the question \emph{"What is Locality?"}. Essentially, locality is the requirement that we work with $(j^\infty)^* \Omega^{\bullet, \bullet}(J^\infty F)$ where $(j^\infty)^* \colon TJ^\infty F \lra T(\FF \times M)$.
...
\begin{definition}[Local differential forms]
  ...
  \begin{equation}\left( \Omega^{\bullet, \bullet}_{loc} (\FF \times M) := ((j^\infty )^* \Omega^{\bullet, \bullet} (J^\infty F), d, \delta \right) \end{equation}
  such that
  \begin{equation}\delta((j^\infty)^* \alpha) := (j^\infty)^* d_V \alpha, \quad \quad d((j^\infty)^* \alpha) := (j^\infty)^* d_H \alpha \end{equation}
\end{definition}
...
\begin{definition}[Local derivations]
  ...
  \begin{equation}Df = (\chi^\infty f^\infty) \circ j^\infty \end{equation}
...
\begin{equation}(j^\infty)^* x^i \equiv x^i, \quad \quad \quad (j^\infty)^* u^\alpha_I \equiv u^\alpha_I \end{equation}
...
\begin{equation}\omega = \omega^{I_1 ... I_p}_{\alpha_1 ... \alpha_p i_1 ... i_q} \delta u^{\alpha_1}_{I_1}\wedge ... \wedge \delta u^{\alpha_p}_{I_p} \wedge dx^{i_1} \wedge ... \wedge dx^{i_q}  \end{equation}
...
\begin{equation}\lie{X} \omega = \imath_X \circ \delta \omega + \delta \circ \imath_X \omega \end{equation}
...
\begin{definition}[Classical Lagrangian Field Theory]
   A \textbf{Classical Lagrangian Field Theory} is a triple $(M, F, L)$ where $F \atob{\pi} M$ is a smooth fibre bundle and $L \in \Omega^{0, top}_{loc}(\FF \times M)$ called a \textbf{Lagrangian (density)}. We call the integral
   \begin{equation}S = \int_M L \end{equation}
   the \textbf{action functional}.
\end{definition}

This means
\begin{equation}L = (j^\infty)^* L^\infty \quad \text{for} \quad L^\infty \in \Omega^{0, top}_{loc}(J^\infty F) \end{equation}
and thus
\begin{equation}L^k \left(x^i, u^\alpha, u^\alpha_{i_1}, ..., u^\alpha_{i_1 ...i_n} \right) dx^1 ... dx^n\end{equation}
If we evaluate $L$ at $\phi$, we obtain
\begin{equation}L(\phi) = L\left(x^i, \phi^\alpha, \dell{\phi^\alpha}{x^i}, ..., \dell{^k \phi^\alpha}{x^{i_1} ... \partial x^{i_n}}\right) \end{equation}
...
Thus a source form, if evaluated on a vectorfield, depends only on the projection of that vectorfield to on on $F$. This definition is a working version and does not really explain itself. The following theorem sheds some light onto the situation:

\begin{theo}
  Let $(M,F,L)$ be a Lagrangian field theory. Then there exists a unique source form $EL \in \Omega^{1,top}_{loc}(\FF \times M)$ and two unique "boundary" forms
  \begin{equation}\alpha \in \Omega^{1,top-1}_{loc}(\FF \times M), \quad \quad \omega \in \Omega^{2,top-1}_{loc}(\FF \times M) \end{equation}
  such that
  \begin{itemize}
...
\begin{equation}E_\alpha  \left(x^i, \phi^\beta, \dell{\phi^\beta}{x^i}, ..., \dell{^k \phi^\beta}{x^{i_1} ... \partial x^{i_n}}\right) = 0 \end{equation}
...
\label{def:strictly}
  A vector field $v\in \VF(J^\infty F)$ is called \textbf{strictly vertical} or \textbf{strictly horizontal}, iff $[\imath_v, d_H] = 0$ or $[\imath_v, d_V] = 0$ respectively.
\end{definition}
...
\begin{proof}
  First note that $[\imath_v, d_V] = 0$ iff it annihilates functions and $1$-forms generated by $\{d_H x^i, d_V u^\alpha_I \}$.
  \begin{align}
    ...
    &\Leftrightarrow \quad v = v^i \dell{}{x^i} + u^\alpha_{I,i} v^i \dell{}{u^\alpha_I} = v^i D_i
  \end{align}
...
\begin{definition}
  A vector field $\xi \colon \FF \lra T \FF$ is said to \textbf{project} to a vector field $v$ on $J^\infty F$, iff the following diagram commutes:
  \begin{center}
  \begin{tikzcd}[sep=huge]
    \FF \times M \arrow[d, "j^\infty"']  \arrow[r, "\xi \times \id"] & T \FF \times M \arrow[d, "Tj^\infty"] \\
    J^\infty F \arrow[r, "v"] & T J^\infty F
  \end{tikzcd}
  \end{center}
\end{definition}

\begin{theo}
  A vector field $v$ on $J^\infty F$ is strictly vertical, iff there exists a local vector field $\xi \in \Gamma(\FF, T \FF)$ that projects to it. Moreover $\xi$ is unique and called an \textbf{evolutionary} vector field.
\end{theo}
...
\label{def:LocalSymmetry}
  ...
  \begin{equation}\lie{\xi} L = \imath_\xi \delta L = dA_\xi \end{equation}
...
\label{theo:Noether_first}
  ...
  \begin{equation}j := A_\xi - \imath_\xi \alpha \ \in \Omega^{0,top-1}_{loc}(\FF\times M) \end{equation}
  is \textbf{on shell}, i.e.
  \begin{equation}d j = 0 \mod(EL) \end{equation}
...
\begin{example}[\textbf{Classical Mechanics}]
  ...
  \begin{align}
    ...
    &= \left( \dd{}{t} \left( m \ \delta q \cdot \dot q \right) - \delta q \left( m \ddot q + \nabla V \right) \right) \wedge dt\\
    &= d \left( m \ \delta q \cdot \dot q \right) - \left( m \ddot q + \nabla V \right) \delta q \wedge dt
  \end{align}
  Thus we have $\alpha = m \dot q^i \delta q_i$ and $E_q = m \ddot q + \nabla V$. Now if we consider a time translation $t \lmap t + \epsilon$ we find the induced evolutionary vector field $\xi = - \dot q^i \dell{}{q^i} - \ddot q^i \dell{}{\dot q^i} ...$\\
  \textbf{Exercise:} Show that $j_\xi$ is an energy, i.e. $j = m \ \dot q \cdot \dot q + V(q)$.
  \begin{proof}
    First note $\imath_\xi \alpha = - m \dot q^i \dot q_i$ and thus let $A_\xi = V(q)$. We obtain $j = V(q) + m \dot q^i \dot q_i$.
  \end{proof}
\end{example}
...
Consider a smooth, oriented, compact manifold $M$ equipped with a (pseudo-)Riemannian metric $g$. This metric and the orientation define a volume form on $M$ given by $\vol_g := \sqrt{|\det(g)|} dx^1 \wedge ... \wedge dx^n$. This so-called Riemannian volume form has this form independently of the choice of local coordinates.

\begin{definition}[Hodge Duality]
  The \textbf{Hodge-Duality map} (Hodge-$*$) is the $\Ci(M)$-linear map
  \begin{equation}* \colon \Omega^k(M) \lra \Omega^{n-k}(M) \end{equation}
  uniquely determined by the equation
  \begin{equation}\alpha \wedge * \beta = \langle \alpha, \beta \rangle_g \vol_g \end{equation}
\end{definition}

The Hodge star acts on a local basis as
\begin{equation}*(dx^1\wedge ... \wedge dx^n) = \frac{\sqrt{|\det(g)|}}{(n-k)!} g^{i_1 j_1} ... g^{i_k j_k} \epsilon_{j_1 ... j_k} dx^{j_k+1} ... dx^{j_n} \end{equation}
...
\label{def:codifferential}
  ...
  \begin{equation}\delta_g := \Omega^k(M) \lra \Omega^{k-1}(M) \end{equation}
  defined as
  \begin{equation}\delta_g := (-1)^{n(k-1)+1} \sgn(g) * d * \end{equation}
  Using the codifferential, one can define the \textbf{Laplace-deRham operator}
  \begin{equation}\Delta_g := (d+\delta_g)^2 = d \delta_g + \delta_g d \end{equation}
\end{definition}
...
\begin{definition}[Scalar Field Theory]
  Let $(M,g)$ as above and $\FF= \Ci(M)$ seen as the sections of a trivial $\RR$-bundle. Then (a.k.a.\ non-interacting) \textbf{free Scalar field theory} on $\FF$ is defined using the Lagrangian
  \begin{equation}L = \frac{1}{2} d \phi \wedge * d\phi \equiv  g^{\mu\nu} \partial_\mu \phi \partial_\nu \phi \sqrt{|\det(g)|} d^n x \end{equation}
\end{definition}
...
\begin{equation}\Delta_g \phi = 0 \end{equation}

\begin{rem}
  One can add "potential (a.k.a.\ interaction) terms" to the Lagrangian. A common example would be $V(\phi) = \frac{m^2}{2} \phi^2 + \frac{\beta}{4} \phi^4$. Note furter that our theory \emph{requires} a metric $g$ on $M$ and is thus a "Non-topological field theory".
\end{rem}
...
\begin{definition}[Pure, d-dimensional Yang-Mills-Theory]
  ...
  \begin{equation}F_A = dA + \frac{1}{2}[A,A] \end{equation}
  Assume for simplicity that $P \lra M$ is a trivial bundle and $A \in \Omega^1(P,\mathfrak{g})$ descends to a $\mathfrak{g}$-valued $1$-form on $M$. Then a \textbf{Pure Yang-Mills-Theory} is the Lagrangian field theory given by $\FF = \mathcal{A}$ (space of principal connections on the principal bundle) and the Lagrangian (density)
  \begin{equation}L_{YM} = \frac{1}{2} \Tr[F_A \wedge * F_A] \end{equation}
\end{definition}

\begin{ex}
  As an exercise, you can show that the variation of $L_{YM}$ is given by
  \begin{equation}\delta L_{YM} = \Tr[d_A * F_A \wedge \delta A + d(\delta A * F_A)] \end{equation}
  Remember that $d_A \phi = d\phi + A \phi$ for $\phi \in \Ci(M, R)$ and $R$ a representation of $y$. For any $\beta \in \Omega^1(M,\mathfrak{g})$ one has $d_A \beta = d\beta + [A, \beta]$ where the latter term denotes the adjoint action/representation.
\end{ex}
...
\begin{definition}[Yang-Mills-Theory with scalar "matter"]
  ...
  \begin{equation}L^{scalar}_{YM} = \Tr[F_A \wedge * F_A] + \langle d_A \phi, *d_A \phi \rangle_R \end{equation}
  ...
  \begin{equation}+ \text{mass terms} \ \frac{m^2}{2} \langle \phi, \phi \rangle_R + \beta \phi^4\end{equation}
...
\begin{equation}\delta L^{1st}_{YM} = (F_A - *B) \delta B + d_A B \delta A + d(B \wedge \delta A) \end{equation}
