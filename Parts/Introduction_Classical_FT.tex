...
\begin{proof}
  The proof is left as an exercise to the reader. \emph{Hint:} Check on the generators $\{x^i, u^\alpha_I\}$ and $\{dx^i, d_V u^\alpha_I\}$.
\end{proof}
\end{lem}
...
\begin{proof}
  ...
  \begin{align*}
    d_j &= d(A_\xi - \imath_\xi \alpha) = \lie{\xi} L - d \imath_\xi \alpha = \lie{\xi} L + \imath_\xi d \alpha\\
...
\begin{align*}
  \Gamma(M,W) &\lra \VF_{ev}(\FF\times M) \times \Omega^{0,top-1}_{loc}(\FF\times M)\\
  w &\lmap (\xi_w, A_{\xi_w})
\end{align*}
such that
