...
\begin{definition}[Shifted Vector Spaces]
  Let $V^\bullet$ be a graded vector space. We \textbf{shift} $V^\bullet$ by $k \in \ZZ$ by defining the \textbf{shifted vector bundle}
  \begin{equation}
    V[k] := \bigoplus_{i \in \ZZ} V[k]^i = \bigoplus_{i \in \ZZ} V^{k+i}
  \end{equation}
  Given a vector space $V^\bullet$ such that $V^i = \{0\}$ for all $i \neq p$, we call $V^\bullet$ \textbf{concentrated in $p$}. In particular for any vector space $V$, the shifted vector space $V[k]$ is concentrated in degree $-k$. For a dual space $V^*$ we obtain
  $$ V^*[k] = \bigoplus_{i \in \ZZ} V^*[k]_i = \bigoplus_{i \in \ZZ} (V^*)_{k+i} = \bigoplus_{i \in \ZZ} (V^{k+i})^* $$
\end{definition}
...
\begin{itemize}
  ...
  Where we denoted by $(x^i)_{i=1}^n$ the coordinates on $\fg[1]$ dual to the basis $(e_i)^n_{i=1}$. As an interesting exercise, prove that $Q$ is indeed cohomological, i.e. of degree $1$ and $Q^2=0$.
\end{itemize}
\end{example}
