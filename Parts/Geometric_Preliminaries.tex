...
\begin{definition} (Graded Morphisms)\\
 Let $V^\bullet, W^\bullet$ be graded vector spaces.  A \textbf{graded morphism} $\phi: V^\bullet \ra W^\bullet$ is  a collection linear maps $\phi^k: V^k \ra W^k$ \ $\forall k \in \ZZ$. We extend this notion to that of a graded morphism of \textbf{degree $r$} by requiring that $\phi^k: V^k \ra W^{k+r}\ \forall k \in \ZZ$. In the case $V^\bullet = W^\bullet$ we call $\phi$ a \textbf{graded endomorphism} (of degree $r$, respectively).
\end{definition}
...
\begin{itemize}
  ...
  \item The two spaces $(\bigwedge^\bullet V, \wedge)$ and $(\Omega^\bullet(M), \wedge)$ are associative commutative graded algebras as it can be readily deduced from the definition of the wedge-product.
...
\end{multicols}
  \begin{proof}
    We only need to prove the claims for functions and exact $1$-forms as all of the above are local operators. First we note that $[d,d]=0$ simply due to $d \bullet d = 0$. For the second identity we note that for functions
    $$ \lie{X}(f) = X(f) = \imath_X df = [\imath_X, d]f $$
    Now for exact $1$-forms we obtain
    $$ [d, \imath_X] df = d\bullet \imath_X df = d \bullet \lie{X}f = \lie{X} df $$
...
$$ \lie{X} \omega = d \bullet \imath_X \omega $$
...
\begin{prop}
  ...
  \begin{proof}
    ...
    Now we further note
    \begin{align*}
      d\bullet \imath_{X_0}\ \omega(X_1, ..., X_k) &= \imath_{X_k} ... \imath_{X_1} \bullet d\bullet \imath_{X_0} \omega\\
...
\begin{definition} (Presheaf)\\
  ...
  \begin{itemize}
    ...
    \begin{itemize}
      ...
      $$ \rho_{UW} = \rho_{VW} \bullet \rho_{UV} $$
...
\begin{definition} (Morphisms of Sheaves)\\
  ...
  \end{tikzcd}
  \]
  Which sums up to saying $\rho^\prime \bullet \phi = \phi \bullet \rho$.
\end{definition}
...
\begin{align}
  \rho^X \bullet \psi = \psi \bullet \rho^Y
\end{align}
...
\begin{rem}
  ...
  $$ [X,Y] := X \bullet Y - (-1)^{kl} \ Y\bullet X $$
...
$$ \lie{X}\omega := \imath_X \bullet d\omega + (-1)^{|X|} d \bullet \imath_X \omega $$
...
\begin{align*}
  ...
  \lie{E} dx^i &= d \bullet \lie{E}(x^i) = |x^i| dx^i
\end{align*}
...
\begin{proof}
  We easily show that
  $$ k \cdot \omega = \lie{E} \omega = d\bullet \imath_E\omega \quad \Rightarrow \quad \omega = d\left( \frac{1}{k} \imath_E \omega \right) $$
\end{proof}
\end{lem}
...
\begin{proof}
  First, we note that
  \begin{align*}
    [E,X] = lX, \quad \quad \lie{X}\omega = d \bullet \imath_X \omega = 0, \quad \quad \lie{E}\omega = k\omega
  \end{align*}
  Now we can define $H:=\imath_E\imath_X \omega$ to obtain
  \begin{align*}
    dH &= d \bullet \imath_E\imath_X \omega = \lie{E}\imath_X\omega - \imath_E \bullet d\imath_X\omega\\
...
\begin{definition} (Fibre Bundles and Sections)\\
  ...
  Now a \textbf{section} (or cross section) of a bundle $(E,\pi,M)$ is a smooth map $s:M\ra E$ satisfying the section property $\pi \bullet s = id_M$.
\end{definition}
...
\begin{proof}
  Let $\sigma:M\ra P$ be a section such that $\pi \bullet \sigma(x) = x$ for all $x \in M$. Now for all $p\in P$ there exists a $\chi_\sigma(p) \in G$ such that
  ...
  \end{align*}
  Now note that $pr_1 \bullet u_\sigma = \pi$ and further $u_\sigma(pg) = u_\sigma(p)g$. Thus $u_\sigma$ is a principal bundle map and conversely we can define $h: M\times G \longrightarrow P$ as a principal bundle map via
  \begin{align*}
...
\begin{definition}
  ...
  such that $\partial_n \bullet \partial_{n+1} = 0$ for all $n \in \NN$. In other words: $\im \partial_{n+1} \subseteq \ker \partial_n$
\end{definition}
...
(Chain maps)\\
  ...
  $$ \partial_{2n} \bullet f_n = f_{n-1} \bullet \partial_{1n} $$
\end{definition}
...
\begin{definition}
  ...
  \begin{itemize}
    ...
    $$ p:B \lra A $$
    such that $p\bullet i = 1_A: A\lra A$
    \item[2.] There is a homomorphism
    $$ s:C \lra B $$
    such that $j \bullet s = 1_C: C\lra C$
...
\begin{itemize}
  ...
  \item[$3\ra 1/2$] Here we use $\psi,i,j$. Define $p(b):= pr_1 \bullet \psi(b)$ and $s(c):= \psi^{-1}\bullet (j^\prime)^{-1}(c)$. Now note that $p \bullet i = id_A$ due to
  $$ p(i(a)) = pr_1(\psi(i(a))) = pr_1(i^\prime(a)) = a $$
  We further have $j \bullet s = id_C$ due to
  $$ j \bullet s(c) = j(\psi^{-1}(0,c)) = j^\prime(0,c) = c $$

  \item[$2\ra 1$] The last thing we need to show uses $s,i,j$. We define $p(b) := i^{-1}(b - s\bullet j(b))$. This
\end{itemize}
\end{proof}
\end{lem}
...
\begin{definition}
  ...
  such that $\delta^n \bullet \delta^{n-1} = 0$ for each $n\in \NN$, namely $\im (\delta^{n-1}) \subseteq \ker (\delta^n)$
\end{definition}
...
\begin{align*}
  ...
  \phi &\lmap \alpha^*(\phi) = \phi \bullet \alpha
\end{align*}
...
These maps automatically satisfy $\delta^n \bullet \delta^{n-1} = 0$ due to the corresponding equation for $\partial_n$. Now there exists a theorem, which is out of the scope of this lecture, but can be found in \source{AlgebraicTopology}, that states that the cohomology groups of $C_\bullet$ under $G$ denoted by $\mathcal{H}^n(C;G)$ are fully determined by $G$ and the homology groups $\mathcal{H}_n(C_\bullet)$.
...
\begin{rem}
  ...
  $$ f^n \bullet \delta_1^{n-1} = \delta_2^{n-1} \bullet f^{n-1} $$
